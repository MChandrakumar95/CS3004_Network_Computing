...
class BankTransactionState {
	...
	String processInput(String InputData) {
		...
		case CLIENT_REQUEST_STATE:
			if (InputData.equalsIgnoreCase("Add_Funds") || InputData.equalsIgnoreCase("Withdraw_Funds")
					|| InputData.equalsIgnoreCase("Transfer_Funds")) {
				try {
		...
		case TRANSACTION_AMOUNT_STATE:
			if (InputData != null) {
				try {
					...
					WindowLogger.log(TAG + " - " + "Transaction Amount: " + transactionAmount, Process.SERVER);
					output = transaction();
	...
	private String transaction() {
		...
		try {
			...
			outputLine = bankFundsCurrentStateObject.triggerTransactionJob(transactionID, ClientID, TransferToClientID,
					transactionAmount);
			bankFundsCurrentStateObject.releaseLock();
			WindowLogger.log(TAG + " - " + "Transaction output: " + outputLine, Process.SERVER);
		} catch (InterruptedException e) {
			WindowLogger.log("Failed to get lock when reading:" + e, Process.SERVER);
			outputLine = "ERROR";
		}
		return outputLine;
	}
